...
- **Static file serving is locked down.** `serveFromRoot` in `src/server.js` resolves and then rejects any path that escapes its root (`/assets/*` → `public/assets`, `/extension/*` → `extension/`). Named routes (`/`, `/overlay`) map to specific files. Keep new file routes going through `serveFromRoot` or an explicit allowlist.
