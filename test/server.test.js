...
test("POST /api/comments/show accepts a manual operator message", async (t) => {
  const server = await startTestServer();
  const socket = new WebSocket(server.wsUrl);
  await new Promise((resolve) => socket.addEventListener("open", resolve, { once: true }));
  t.after(async () => {
...
test("POST /api/comments/show rejects a manual message without text", async (t) => {
  const server = await startTestServer();
  t.after(async () => {
    await server.close();
  });
  ...
  const response = await fetch(`${server.baseUrl}/api/comments/show`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ manual: true, message: "   " }),
  });
